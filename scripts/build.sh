...
assemble_parts() {
    ...
    : > "$destination"
    for part in "$@"; do cat "$part" >> "$destination"; printf '\n' >> "$destination"; done
}
assemble_parts "$BUILD/common/functions.sh" "$MODULE"/src/functions/*.sh
assemble_parts "$BUILD/neorender" "$MODULE"/src/menu/*.sh
assemble_parts "$BUILD/neorenderctl" "$MODULE"/src/controller/*.sh
rm -rf "$BUILD/src"
for file in README.md README_RU.md CHANGELOG.md RELEASE_NOTES.md SECURITY.md LICENSE; do cp "$ROOT/$file" "$BUILD/$file"; done
mkdir -p "$BUILD/system/bin"
cp "$BUILD/neorender" "$BUILD/system/bin/neorender"
cp "$BUILD/neorenderctl" "$BUILD/system/bin/neorenderctl"
chmod 0755 "$BUILD/neorender" "$BUILD/neorenderctl" "$BUILD/system/bin/neorender" "$BUILD/system/bin/neorenderctl"
...
(cd "$BUILD" && find . -type f ! -name SHA256SUMS -print | sed 's#^./##' | LC_ALL=C sort | while IFS= read -r rel; do sha256sum "$rel"; done > SHA256SUMS)
